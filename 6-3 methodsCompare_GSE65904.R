...
genelist$PRGs_74[which(genelist$PRGs_74 %in% rownames(ExprMatrix)==FALSE)]
rownames(ExprMatrix)[ rownames(ExprMatrix)=="PJVK"] <- "DFNB59"
rownames(ExprMatrix)[rownames(ExprMatrix)=="GSDME"] <- "DFNA5"
rownames(ExprMatrix)[rownames(ExprMatrix)=="LAMTOR3"] <- "MAPKSP1"
rownames(ExprMatrix)[rownames(ExprMatrix)=="LAMTOR4"] <- "C7orf59"
rownames(ExprMatrix)[rownames(ExprMatrix)=="LAMTOR5"] <- "HBXIP"
rownames(ExprMatrix)[rownames(ExprMatrix)=="IST1"] <- "KIAA0174"
rownames(ExprMatrix)[rownames(ExprMatrix)=="MAP3K20"] <- "ZAK"
c("VPS24", "CHMP3","CGI-149", "NEDF") %in% rownames(ExprMatrix)   # NO VPS24
...
for(j in colnames(GSE65904_m)[2:13]){
