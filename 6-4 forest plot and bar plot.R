...
# unicox_order$Risk_group <- factor(unicox_order$Risk_group, levels = c("Protect", "Risk", "Not_sig"))
...
pdf("d:/Work/Projects/BMC_Revised/Results/08.methods_compare/forestplot.pdf", width = 7, height = 8)
ggplot(unicox_order[1:24,], aes(HR, method_dataset)) + 
  # facet_grid(~dataset) +
  geom_errorbarh(aes(xmax = HR.95L, xmin = HR.95H,col= Risk_group),height=0.3,size=0.7) +
  geom_point(shape=18, aes(col=Risk_group, size = P_value))  +
  geom_vline(aes(xintercept = 1),color="darkgrey",linetype = "dashed",size=0.8) + 
  ...
  scale_color_manual(values = c("#FFDC91FF","#0072B5FF","#BC3C29FF"))+
  theme_bw() 
ggplot(unicox_order[25:48,], aes(HR, method_dataset)) + 
  # facet_grid(~dataset) +
  geom_errorbarh(aes(xmax = HR.95L, xmin = HR.95H,col= Risk_group),height=0.3,size=0.7) +
  geom_point(shape=18, aes(col=Risk_group, size = P_value))  +
  geom_vline(aes(xintercept = 1),color="darkgrey",linetype = "dashed",size=0.8) + 
...
pdf("d:/Work/Projects/BMC_Revised/Results/08.methods_compare/barplot.pdf", width = 10, height = 8)
ggplot(unicox_order, aes(x=gene, y=Concordance, fill = dataset)) +
  geom_bar(stat="identity", position="dodge", width = .6)+
  geom_hline(yintercept=min(unicox_order$Concordance[unicox_order$gene == "PRGs_prot"])) +
  coord_flip() +
